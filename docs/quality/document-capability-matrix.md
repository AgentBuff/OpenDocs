...
| Inline style | RichText runs, `patchInlineRange` | toolbar and editable projection | Rust/TS unit coverage; cross-block batched bold via `e2e/document/selection.spec.ts`; font catalog round-trip via `test/fonts.test.ts` + `e2e/document/fonts.spec.ts` | partial — range mapping unified; grouped font catalog (Chinese/Latin/mono CSS stacks) persisted verbatim; engine fix: `fontFamily` no longer mis-validated as a color; visual coverage remains |
